...
{
    $q = isset($_GET['q']) ? filter_str($_GET['q']) : '';
    $sort = array();
    $sort[2] = ' ORDER BY product.create_at DESC ';
    ...
    $sub_condition = '';
    if (!empty($brand_id)) {
        $sub_condition = "AND brand = {$brand_id}";
    }
    $sql = "SELECT product.slug,product.name,product.product_id,product.price,product.discount, media.url FROM tbl_product as product INNER JOIN tbl_media as media ON media.media_id = product.thumb WHERE cat_id = {$cat_id} {$sub_condition} AND product.name like '%{$q}%'";
