...
function update_info_cart()
{
    ...
    $result = db_fetch_row("SELECT Percent FROM `tbl_discount` WHERE Code = '{$discountCode}'");

    $cart['info']['qty'] = $total_qty;
    $cart['info']['total'] = $total_price;
    $cart['info']['discount_code'] = $discountCode;
    $cart['info']['percent'] = $result['Percent'] * $cart['info']['total'] / 100;
    echo $result;
    set_session(array('cart' => $cart));
}
